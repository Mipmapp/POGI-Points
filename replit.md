...
- **GeofenceMap UI overhaul (April 29, 2026):** Replaced the teardrop pin with a centered "bullseye" round marker (iconAnchor `[15,15]`) so the marker visually centers on the geofence circle. Disabled Leaflet's default zoom + attribution controls and re-added a custom zoom control at `topright` so it no longer overlaps the Street/Satellite tile switcher. Moved the LIVE pill below the zoom control. Bottom distance bar now stacks vertically on mobile (`flex-col sm:flex-row`). A subtle "Map data © OpenStreetMap/Esri contributors" footer renders below the map; the in-map attribution watermark is hidden via CSS to keep the surface clean.
- **Admin Contributions UX (April 29, 2026):** (a) When the student search returns exactly one match, `searchStudent()` now auto-selects it and dispatches a success toast — no extra tap needed. (b) New "Paid On" date filter section above the existing year/program/status/college filters: chip buttons for Yesterday / Today (with live pulse dot) / Tomorrow plus a native `<input type="date">` for any custom day. The chips toggle off on a second tap, and `filteredContributions` now compares each `c.paid_at` against the picked local YYYY-MM-DD. A live "Showing N payments collected on Mon, Apr 29 2026" summary card surfaces the impact of the filter. Mobile-responsive (`flex-wrap`, no `xs:` breakpoint usage) and matches the ssaam-dark→ssaam-light gradient theme.
