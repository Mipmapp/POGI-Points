...
- **Notifications System:** Global notification collection (`notifications`) shared across all colleges. Only the super Admin can post, edit, or delete notifications. All authenticated users see the same announcements. Like and seen tracking use the global `notification_seen` collection.
