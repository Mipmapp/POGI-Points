...
- **Attendance:** RFID code support for attendance. Students receive email notifications upon RFID verification. Supports both "Single Session" (2-in-a-day) and "Dual Session" (4-in-a-day) attendance tracking modes. RFID scanner supports both RFID scan and manual Student ID input modes. Admins can mark RFID cards as 'Unreadable'. **Session-Level Logs:** Admin dashboard displays attendance logs per session (not per event), with Logs button at session level. User dashboard shows expandable event folders with per-session attendance history including check-in/check-out times.
