...
- **Face ID (Biometric Check-In):** Optional self-service face enrollment for students (capped at 3 faces per student) via the new `StudentFaceID.vue` panel in the student profile. Admin attendance kiosk gains a Face ID mode (`FaceScannerKiosk.vue`) selectable via a tab toggle next to the RFID scanner — uses face-api.js entirely client-side, captures a 128-float descriptor, and posts it to `POST /apis/attendance/sessions/:sessionId/check-face`. The backend matches the descriptor (Euclidean distance, threshold 0.45) against enrolled students in the active college and delegates to the unified `sessionAttendanceCheck` handler so behaviour is identical to RFID. Student face descriptors are stripped from generic Student JSON responses; only the dedicated `/apis/students/face` endpoints (GET/POST/DELETE under `studentAuthWithToken`) ever return them.
