<template>
  <transition name="terminal-fade">
    <div v-if="visible" class="fixed inset-0 z-[100] flex items-center justify-center bg-purple-50 font-mono">
      ...
      <div class="relative w-full max-w-2xl p-8 border border-purple-200 bg-white/90 backdrop-blur-2xl shadow-[0_20px_50px_rgba(168,85,247,0.15)] rounded-2xl overflow-hidden">
        <!-- SSAAM Branding Corner -->
        <div class="absolute top-0 right-0 p-4 opacity-10">
          <div class="text-[40px] font-bold text-purple-900 select-none tracking-tighter">SSAAM</div>
        </div>

        <!-- Terminal Header -->
        <div class="absolute top-0 left-0 right-0 h-10 bg-gradient-to-r from-purple-100 to-pink-50 border-b border-purple-100 flex items-center px-4 gap-2">
          ...
          <span class="ml-3 text-[10px] text-purple-900/40 tracking-[0.3em] uppercase font-bold">Secure Access Protocol v2.0</span>
        </div>
        ...
        <div class="mt-6 space-y-3 text-sm md:text-base min-h-[220px] relative z-10">
          <div v-for="(line, index) in visibleLines" :key="index" class="flex items-start">
            <span class="mr-3 text-purple-400 font-bold">>></span>
            <span class="typing-text text-purple-900 font-medium">{{ line }}</span>
          </div>
          ...
          <div v-if="showProgress" class="mt-10 space-y-2">
            <div class="flex justify-between items-end">
              <div class="flex flex-col">
                <span class="text-[10px] uppercase tracking-widest text-purple-900/40 font-bold">System Status</span>
                <span class="text-xs text-purple-900 font-bold">{{ message }}</span>
              </div>
              <span class="text-xl font-bold italic text-pink-500 tabular-nums">{{ progress }}%</span>
            </div>
            <div class="h-2 w-full bg-purple-100 rounded-full relative overflow-hidden">
              <div 
                class="h-full bg-gradient-to-r from-purple-600 to-pink-500 rounded-full transition-all duration-300 relative" 
